...
class L_Entity(NodePath):

    rotation_directions = (-1,-1,1)
    default_shader = None
    default_values = {
        # 'parent':scene,
        'name':'l_entity', 'enabled':True, 'eternal':False, 'position':Vec3(0,0,0), 'rotation':Vec3(0,0,0), 'scale':Vec3(1,1,1), 'model':None, 'origin':Vec3(0,0,0),
    ...
    def __init__(self, add_to_scene_entities=True, **kwargs):
        ...
        if L_Entity.default_shader:
            self.shader = L_Entity.default_shader
        ...
        self.scripts = []   # add with add_script(class_instance). will assign an 'l_entity' variable to the script.
        self.animations = []
        self.hovered = False    # will return True if mouse hovers l_entity.
    ...
    def collider(self, value):
        ...
        if value == 'box':
            if self.model:
                ...
                self._collider = BoxCollider(l_entity=self, center=_bounds.center, size=_bounds.size)
            else:
                self._collider = BoxCollider(l_entity=self)
            self._collider.name = value

        elif value == 'sphere':
            self._collider = SphereCollider(l_entity=self, center=-self.origin)
            self._collider.name = value

        elif value == 'capsule':
            self._collider = CapsuleCollider(l_entity=self, center=-self.origin)
            self._collider.name = value

        elif value == 'mesh' and self.model:
            self._collider = MeshCollider(l_entity=self, mesh=None, center=-self.origin)
            self._collider.name = value

        elif isinstance(value, Mesh):
            self._collider = MeshCollider(l_entity=self, mesh=value, center=-self.origin)

        elif isinstance(value, str):
            m = load_model(value)
            if not m:
                return
            self._collider = MeshCollider(l_entity=self, mesh=m, center=-self.origin)
            self._collider.name = value
    ...
    @property
    def X(self):    # shortcut for int(l_entity.x)
        return int(self.x)
    @property
    def Y(self):    # shortcut for int(l_entity.y)
        return int(self.y)
    @property
    def Z(self):    # shortcut for int(l_entity.z)
        return int(self.z)

    @property
    def world_rotation(self):
        rotation = self.getHpr(scene)
        return Vec3(rotation[1], rotation[0], rotation[2]) * L_Entity.rotation_directions

    @world_rotation.setter
    def world_rotation(self, value):
        self.setHpr(scene, Vec3(value[1], value[0], value[2]) * L_Entity.rotation_directions)
    ...
    def rotation(self, value):
        ...
        self.setHpr(Vec3(value[1], value[0], value[2]) * L_Entity.rotation_directions)
    ...
    def forward(self): # get forward direction.
        return Vec3(*self.parent.getRelativeVector(self, (0, 0, 1)))
    @property
    def back(self): # get backwards direction.
        return -self.forward
    @property
    def right(self): # get right direction.
        return Vec3(*self.parent.getRelativeVector(self, (1, 0, 0)))
    @property
    def left(self): # get left direction.
        return -self.right
    @property
    def up(self): # get up direction.
        return Vec3(*self.parent.getRelativeVector(self, (0, 1, 0)))
    @property
    ...
    @property
    def billboard(self): # set to True to make this L_Entity always face the camera.
    ...
    def get_position(self, relative_to=scene):  # get position relative to on other L_Entity. In most cases, use .position instead.
        return self.getPos(relative_to)


    def set_position(self, value, relative_to=scene): # set position relative to on other L_Entity. In most cases, use .position instead.
    ...
    def rotate(self, value, relative_to=None):  # rotate around local axis.
        ...
        self.setHpr(relative_to, Vec3(value[1], value[0], value[2]) * L_Entity.rotation_directions)


    def add_script(self, class_instance):
        if isinstance(class_instance, object) and type(class_instance) is not str:
            class_instance.l_entity = self
    ...
    def has_ancestor(self, possible_ancestor):
        ...
        p = self
        if isinstance(possible_ancestor, L_Entity):
            # print('ENTITY')
            for i in range(100):
                if p.parent:
...
#------------
    ...
    def intersects(self, traverse_target=scene, ignore=(), debug=False):
        ...
        self.hit.l_entity = next(e for e in scene.entities if e == nP)
...
if __name__ == '__main__':
    ...
    e = L_Entity(model='quad', color=color.orange, position=(0,0,1), scale=1.5, rotation=(0,0,45), texture='brick')

    '''example of inheriting L_Entity'''
    class Player(L_Entity):
    ...
    # test
    e = L_Entity(model='cube', collider='box', texture='shore', color=hsv(.3,1,.5))
    print(repr(e))
    # a = L_Entity()
    # b = L_Entity(parent=a)
