''' FINAL: 
- Merge this code into code for a class so I can merge it with actual abilities in abilities.py'''
...
- Set distance of dash point to interception point (just like placing the end_point at the hit point, we are also making the visual dash_arc stop/calculate at that same position as where the end_point lands)
- Make a perfectly accurate final collision ray point:
    - If any of the main rays hit, Take the closest vertice on the main arc that is closest to this ray hit point, and actually place the end_point on that point!!!
...
end_point = Entity(model='cube', color=color.orange, 
    scale=(4, 4, 4),
...
def ray(origin, destination):
    dash_ray = linecast(
    origin, destination,
    ignore = [player, end_point, dash_arc, camera, ],
    debug = True
    )
    return dash_ray
...
def start_ray(a, b, c):
    ...
    if dash_ray.world_point is not None:
        ...
        end_point.world_position = dash_ray.world_point
